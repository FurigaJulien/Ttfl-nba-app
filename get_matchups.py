...
def get_matchup(player1_name,player2_name,nb_year,season_type):
    ...
    player2 = players.find_players_by_full_name(player2_name)[0]['id']
    ...
    data = PlayerCareerStats(player_id=player2).get_data_frames()[0]
    dict_player_2={}
    for row in data.itertuples():
        if row.SEASON_ID not in list(dict_player_2.keys()):
            dict_player_2[row.SEASON_ID] = []
            dict_player_2[row.SEASON_ID].append(row.TEAM_ID)
        else :
    ...
    for i in range(nb_year):
        ...
        print(season)
        for team_id in dict_player_2[season]:
            ...
            for game_id in set_list_game_ids:
                ...
                time.sleep(0.600) 
                if player2 in data_match['PLAYER_ID'].values and player1 in data_match['PLAYER_ID'].values:
                    ...
                    try:
                        ...
                        vs_team.append(data_match[data_match['PLAYER_ID']== player2].iloc[0]['TEAM_ABBREVIATION'])
